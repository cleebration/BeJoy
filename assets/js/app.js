...
/* Klick auf "Lesen →", Beitragskarte oder AI-Karte öffnet den Reader */
document.addEventListener('click', e => {
...
/* Footer-Themen-Links → Blog filtern */
document.addEventListener('click', e => {
  const cl = e.target.closest('[data-cat-link]');
  if (!cl) return;
  e.preventDefault();
  const tab = document.querySelector(`.filter-tab[data-cat="${cl.dataset.catLink}"]`);
  if (tab) tab.click();
...
document.getElementById('cookie-reopen')?.addEventListener('click',(e)=>{
  e.preventDefault();
